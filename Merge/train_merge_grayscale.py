"""
Training script for Merge-v0 environment with GrayscaleObservation
"""
...
def create_merge_env_grayscale():
    ...
    # Configure environment for GrayscaleObservation - optimized for Merge
...
def train_merge_grayscale(timesteps=100000, model_name="merge_grayscale"):
    """
    Train PPO agent on Merge-v0 with GrayscaleObservation
    ...
    """
    print("=" * 60)
    print("Training Merge-v0 with GrayscaleObservation")
    print("=" * 60)
    ...
    def make_env():
        env = create_merge_env_grayscale()
    ...
    # Create PPO model with CNN policy and optimized hyperparameters for Merge-v0
    print("\nInitializing PPO model with CnnPolicy...")
    model = PPO(
        "CnnPolicy",
        env,
        learning_rate=2.5e-4,  # Lower for CNN stability in merge scenario
        n_steps=2048,
        batch_size=64,
        n_epochs=10,
        gamma=0.99,
...
if __name__ == "__main__":
    # Train the model
    model, callback = train_merge_grayscale(timesteps=500000, model_name="merge_grayscale")
