"""
Evaluation and plotting script for Merge-v0 DRL models
...
def create_merge_env_grayscale():
    """Create Merge environment with GrayscaleObservation"""
    env = gym.make('merge-v0', render_mode='rgb_array')
    env.unwrapped.config.update({
        "observation": {
...
def plot_learning_curve(training_data_path, save_path, title):
    ...
    # Truncate to first 2000 episodes
    episode_rewards = episode_rewards[:2000]
...
def main():
    ...
    print("=" * 60)
    print("Merge-v0 Model Evaluation and Plotting")
    print("=" * 60)
    ...
    if os.path.exists("models/merge_lidar.zip") and os.path.exists("training_data/merge_lidar_training.json"):
        # Plot learning curve
        print("\n1. Generating learning curve...")
        plot_learning_curve(
            "training_data/merge_lidar_training.json",
            "plots/merge_lidar_learning_curve.png",
            "Merge-v0 Learning Curve (LidarObservation)"
        )
        
        # Evaluate model
        print("\n2. Evaluating trained model...")
        model_lidar = PPO.load("models/merge_lidar")
        eval_rewards_lidar = evaluate_model(model_lidar, create_merge_env_lidar, n_episodes=100)
        
        # Save evaluation data
        with open("evaluation_data/merge_lidar_eval.json", 'w') as f:
            json.dump({
        ...
        plot_violin_performance(
            eval_rewards_lidar,
            "plots/merge_lidar_violin.png",
            "Merge-v0 Performance Test - 100 Episodes (LidarObservation)"
        )
    ...
    if os.path.exists("models/merge_grayscale.zip") and os.path.exists("training_data/merge_grayscale_training.json"):
        # Plot learning curve
        print("\n1. Generating learning curve...")
        plot_learning_curve(
            "training_data/merge_grayscale_training.json",
            "plots/merge_grayscale_learning_curve.png",
            "Merge-v0 Learning Curve (GrayscaleObservation)"
        )
        
        # Evaluate model
        print("\n2. Evaluating trained model...")
        model_grayscale = PPO.load("models/merge_grayscale")
        eval_rewards_grayscale = evaluate_model(model_grayscale, create_merge_env_grayscale, n_episodes=100)
        
        # Save evaluation data
        with open("evaluation_data/merge_grayscale_eval.json", 'w') as f:
            json.dump({
        ...
        plot_violin_performance(
            ...
            "plots/merge_grayscale_violin.png",
            "Merge-v0 Performance Test - 100 Episodes (GrayscaleObservation)"
        )
    ...
    print("  - merge_lidar_learning_curve.png")
    print("  - merge_lidar_violin.png")
    print("  - merge_grayscale_learning_curve.png")
    print("  - merge_grayscale_violin.png")
