# merge Environment Deep Reinforcement Learning

This project trains and evaluates PPO agents on the merge-v0 environment using two different observation types: **Grayscale** and **LiDAR**.
...
```bash
python train_merge_grayscale.py
```

Train a PPO agent with LiDAR observation:
```bash
python train_merge_lidar.py
```
...
```
.
├── train_merge_grayscale.py   # Training script for grayscale observation
├── train_merge_lidar.py       # Training script for LiDAR observation
├── evaluate_and_plot.py         # Evaluation and visualization script
├── models/                      # Saved PPO models (.zip files)
├── logs/                        # TensorBoard event logs
│   ├── merge_grayscale/       # Grayscale training logs
│   └── merge_lidar/           # LiDAR training logs
├── training_data/               # Training metrics (JSON)
│   ├── merge_grayscale_training.json
│   └── merge_lidar_training.json
├── evaluation_data/             # Evaluation metrics (JSON)
│   ├── merge_grayscale_eval.json
│   └── merge_lidar_eval.json
