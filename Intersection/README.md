# Intersection Environment Deep Reinforcement Learning

This project trains and evaluates PPO agents on the Intersection-v0 environment using two different observation types: **Grayscale** and **LiDAR**.
...
```bash
python train_intersection_grayscale.py
```

Train a PPO agent with LiDAR observation:
```bash
python train_intersection_lidar.py
```
...
```
.
├── train_intersection_grayscale.py   # Training script for grayscale observation
├── train_intersection_lidar.py       # Training script for LiDAR observation
├── evaluate_and_plot.py         # Evaluation and visualization script
├── models/                      # Saved PPO models (.zip files)
├── logs/                        # TensorBoard event logs
│   ├── intersection_grayscale/       # Grayscale training logs
│   └── intersection_lidar/           # LiDAR training logs
├── training_data/               # Training metrics (JSON)
│   ├── intersection_grayscale_training.json
│   └── intersection_lidar_training.json
├── evaluation_data/             # Evaluation metrics (JSON)
│   ├── intersection_grayscale_eval.json
│   └── intersection_lidar_eval.json
